...
def split_pdf_to_a4_pages(input_pdf_path: Path, output_pdf_path: Path):
    ...
    num_pages = int(original_height / A4_HEIGHT) + (
        1 if original_height % A4_HEIGHT > 0 else 0
    )
    print(f"  Потребуется страниц A4: {num_pages}")

    if num_pages > 50:
        print(
            f"  Предупреждение: будет создано {num_pages} страниц (это больше ожидаемых 31)"
        )
